...
export class MemoryIndexManager extends MemorySearchOrchestration implements MemorySearchManager {
  ...
  protected async syncAdmitted(
    ...
    const run = async () => {
      // An older pass can clear pending files while reconciliation awaits admission.
      // Refresh the canonical corpus only after that pass settles.
      if (params?.reason === "session-reconcile") {

...
describe("carried memory watch admission regressions", () => {
  ...
  it.each(["targeted", "full"])(
    "reconciles a session replacement arriving during a %s embedding pass",
    async (scope) => {
      const sessionKey = "agent:main:memory:rollover-race";
  ...
  it.runIf(process.platform === "darwin" || process.platform === "win32").each([
    ...
    async ({ resetDirty }) => {
      expect(
        Reflect.get(globalThis, Symbol.for("openclaw.test.memoryNativeWatchFactory")),
      ).toBeUndefined();
